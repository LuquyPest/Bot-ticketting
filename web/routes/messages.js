...
router.post('/', async (req, res) => {
  try {
    ...
    const [ticket] = await query('SELECT id, status FROM tickets WHERE id = ?', [ticket_id]);
...
router.delete('/:id', async (req, res) => {
  try {
    const [msg] = await query('SELECT * FROM scheduled_messages WHERE id = ?', [req.params.id]);
    ...
    await query('DELETE FROM scheduled_messages WHERE id = ?', [req.params.id]);
    res.json({ ok: true });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'Erreur serveur' });
